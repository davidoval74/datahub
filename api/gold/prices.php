...
function load_gold_prices_from_csv_with_php($mysqli, $csvPath) {
    if (!is_file($csvPath)) {
        throw new RuntimeException('CSV de ouro nao encontrado para carga no banco.');
    }
    ...
    $mysqli->query('TRUNCATE TABLE gold_prices');

    $stmt = $mysqli->prepare('INSERT INTO gold_prices (timestamp, price) VALUES (?, ?)');
    if (!$stmt) {
        throw new RuntimeException('Falha ao preparar insert de gold_prices.');
    }

    $handle = fopen($csvPath, 'r');
    if ($handle === false) {
        $stmt->close();
        throw new RuntimeException('Falha ao abrir CSV de ouro para leitura.');
    }
...
try {
    ...
    $extractScript = $pythonDirectory . DIRECTORY_SEPARATOR . 'ExtractGold.py';
    $loadScript = $pythonDirectory . DIRECTORY_SEPARATOR . 'LoadGold.py';
    $csvPath = realpath(__DIR__ . '/../../gold_prices.csv') ?: (__DIR__ . '/../../gold_prices.csv');
    ...
    try {
        ...
        if (strpos($loadErrorText, "No module named 'mysql'") !== false || strpos($loadErrorText, 'No module named mysql') !== false) {
            $loadOutput = load_gold_prices_from_csv_with_php($mysqli, $csvPath);
        } else {
            throw $loadException;
        }
    }
    ...
    send_json(500, ['ok' => false, 'message' => 'Falha ao atualizar/consultar gold_prices.', 'details' => $e->getMessage()]);
}
